...
const CodeBlockWidget = ({ node, updateAttributes }: any) => {
  return (
    <NodeViewWrapper className="code-block-widget my-4 shadow-lg rounded-xl overflow-hidden border border-gray-200">
      <div className="bg-gray-800 px-4 py-2 flex justify-between items-center text-white text-xs font-bold">
        <select
          value={node.attrs.language}
          onChange={(e) => updateAttributes({ language: e.target.value })}
          className="bg-gray-700 text-white rounded px-2 py-1 outline-none border-none cursor-pointer hover:bg-gray-600 transition-colors"
        >
          {languages.map((lang) => (
