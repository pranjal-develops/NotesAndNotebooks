...
const EditPopup: React.FC<EditPopupProps> = ({ note, onClose, onToggleDrawing }) => {
  ...
  const { handleDelete, handleUpdate } = useHandleEvents();
      ...
      >
        ...
        <div className="flex-col">
          ...
          <div className="sticky top-0 z-10 px-6 py-4 flex items-center justify-between">
            <h2 className="text-sm font-bold uppercase tracking-widest text-zinc-700/80 dark:text-white/80 dark-island:text-white/80">Edit Note</h2>
            <button
              type="button"
              onClick={closePopup}
        ...
        <form onSubmit={handleSubmit} className="p-6">
          <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-zinc-200 dark:scrollbar-thumb-zinc-800">
            <div>
              <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 dark-island:text-zinc-300 mb-1">Title</label>
              <input
                type="text"
                value={title}
            ...
            <div>
              <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 dark-island:text-zinc-300 mb-1">Description</label>
              <textarea
                value={description}
          ...
          <div className="mt-8 flex items-center justify-between gap-3 pt-4 border-t border-zinc-100 dark:border-zinc-800 dark-island:border-zinc-800">
            <button
              type="button"
              onClick={DeleteHandler}
              className="px-4 py-2 text-sm font-medium text-red-600 bg-red-50 dark:bg-red-900/20 dark-island:bg-red-900/20 hover:bg-red-100 dark:hover:bg-red-900/30 dark-island:hover:bg-red-900/30 border border-transparent rounded-lg focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 transition-all"
            >
              Delete
            </button>
